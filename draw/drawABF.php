...
if(!$validated)
{
	...
	else
	{
		...
		//prendo l'ultimo e lo confronto con il 4° se hanno lo stesso punteggio lo aggiungo
		$sql="SELECT * FROM judoka
		INNER JOIN pair ON judoka.idJudoka=pair.judoka
		INNER JOIN poule ON pair.poule=poule.idPoule
	...
	$sql="SELECT * FROM judoka 
		INNER JOIN pair ON judoka.idJudoka=pair.judoka
		INNER JOIN poule ON pair.poule=poule.idPoule
		WHERE poule.katatype = :katatype AND poule.tournament = :idTournament AND (type = :typeA OR type = :typeB) AND ($iddanascondere)
		ORDER BY place";
